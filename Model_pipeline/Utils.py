...
class categorical_encoding:
    ...
    def transform(self, d):
        ...
        d : DataFrame
        """
        # Assuming the categorical features are the same as when the encoder was fitted
        transformed_data = self.te.transform(d[self.te.get_feature_names_out()])
        d = pd.concat([d.drop(columns=self.te.get_feature_names_out()), transformed_data], axis=1)
        return d
